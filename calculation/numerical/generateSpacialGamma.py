...
def generate_gamma_variable_z(K_z_t, z_z_t, gamma1=0.003, gamma2=0.0020,
                              delta1=20, delta2=30, z1_min=30, z1_max=80,
                              z2_min=150, z2_max=300):

    print(K_z_t.shape, z_z_t.shape)
    nz, nt = z_z_t.shape
    gamma_zt = np.zeros((nt, nz))
    z1_t = np.zeros(nt)
    z2_t = np.zeros(nt)
    ...
    for t in range(nt):
        ...
        z1 = z_t_min + (0.05 + 0.05 * ratio) * z_t_range         # 地表層厚さ5〜10%
        neutral_thickness = (0.05 + 0.4 * ratio) * z_t_range   # 中立層厚さ5〜45%
...
def plot_theta_profiles_variable_z(gamma_zt, z_z_t, local_times,
                                   z1_t, z2_t, dt=0.1, colors=None):
    nz, nt = gamma_zt.shape#864000
    time_axis = np.arange(nt) * dt #0 ~ 86400
    if colors is None:
    ...
    print("gamma_zt.shape:", gamma_zt.shape)
    print("z_z_t.shape:", z_z_t.shape)
    print("local_times:", local_times)
    print("time_axis[0], time_axis[-1]:", time_axis[0], time_axis[-1])
    if gamma_zt.shape[0] == z_z_t.shape[1]:
        gamma_zt = gamma_zt.T  # (261, 864000)に
    ...
    for i, hour in enumerate(local_times):
        ...
        #idx = int(hour)
        print(f"hour: {hour}, t_sec: {t_sec}, idx: {idx}, z_max: {z_z_t[-1, idx]}")
...
def plot_neutral_layer_thickness(z1_t, z2_t, dt=3600):
    """
    ...
    nt = len(z1_t)
    time_hours = np.arange(nt) * dt / 36000  # 単位を「時間」に変換

    neutral_thickness = z2_t - z1_t

    plt.figure(figsize=(8, 4))
    plt.plot(time_hours, neutral_thickness, color='blue', linewidth=2)
    plt.fill_between(time_hours, neutral_thickness, color='blue', alpha=0.2)
    plt.xlabel("LT [hour]")
    plt.ylabel("neutral thickness [m]")
...
if __name__ == "__main__":
    ...
    #Ks_ = Ks[::steps_per_hour]
    #Ks = np.append(Ks_, Ks[-1])


    nz = 261
    nt = 864000
    #make z_z_t by hypothetical delta z
    appxNalpha = np.sqrt(3.72 / 210 * 4e-3)*np.sin(np.pi/180)
    dz_t = 2*np.pi / nz * np.sqrt(2*Ks/appxNalpha)
    print("appxN_alpha: ", appxNalpha)
    print("dz_t", dz_t.max(), dz_t)


    # z 配列 (261, 864000) を生成
    z_indices = np.arange(nz)[:, None]  # shape (261, 1)
    dz_array = dz_t[None, :]        # shape (1, 864000)

    z_z_t = z_indices * dz_array         # shape (261, 864000)

    print("z_indices.shape: ", z_indices.shape)
    print("dz_array: ", dz_array.shape)
    print("made z_z_t: ",z_z_t.shape, np.max(z_z_t))
    input("one")
    ...
    # --- 3. γ, z₁, z₂ を計算 ---
    gamma_zt, z1_t, z2_t = generate_gamma_variable_z(Ks, z_z_t)
    ...
    gamma_zt = gamma_zt.T
    # --- 5. 指定時刻でプロット ---
    plot_theta_profiles_variable_z(gamma_zt, z_z_t,
                                   local_times=[0, 6, 9, 12, 15, 18],
                                   z1_t=z1_t, z2_t=z2_t)
    plot_neutral_layer_thickness(z1_t=z1_t, z2_t=z2_t, dt=3600)

    #---6 save ---
    filename = "test_spaciotmp_gamma.nc"
    #save_gamma_to_netcdf(gamma_zt, filename)
