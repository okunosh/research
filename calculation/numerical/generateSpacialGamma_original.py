...
if __name__ == "__main__":
    ...
    appxN = np.sqrt(3.72 / 210 * 4e-3)
    dz_t = 2*np.pi / Ks.shape[0] * np.sqrt(2*Ks/appxN)  
    z_z_t = np.linspace(0, 262)*dz_t
    print("made z_z_t: ", max(z_z_t))
    ...
    #gamma_z_t = generate_gamma_from_K(K_z_t)
    #print("gamma_z_t: ", gamma_z_t.shape)
    #plot_gamma_index_simple(gamma_z_t)
    #plt.show()
    #plot_gamma_colormap(gamma_z_t, dt=0.1, z_resolution=4.0, max_time_sec=100.0)
