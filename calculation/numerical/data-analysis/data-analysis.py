...
def run_all(directory: str) -> None:
    ...
    if not np.isfinite(g):
        planet = (attrs_dic.get("planet") or "").lower()
        if "Mars" in planet:
            g = 3.71
        elif "Earth" in planet:
            g = 9.81
        else:
            g = 3.71 if p_surf <= 2_000 else 9.81
        print(f"[warn] invalid global attr g=({g_raw});fallback g={g}")
    ...
    plot_ubar_thetabar(
        ...
        colorbar="each",
        method="pcolormesh",
        save_path=None, #str(out_dir / "time_alt_map_pcolormesh.png"),
    )
