...
def _build_method_suffix(interp_method: str, n_harmonics: int) -> str:
    """補間方法に応じたファイル名サフィックスを作る。"""
    if interp_method == "fourier":
        return f"_fourierN{n_harmonics}"
    elif interp_method == "linear":
        return "_linear"
    else:
        raise ValueError(f"未知の補間方法です: {interp_method!r}")
...
def generate_surface_forcing_and_K_from_MCD(
    ...
    *,
    ...
    1. surface forcing（theta_surf）の NetCDF + 図 を生成
    2. 同じ位相の日変化を持つ K(z,t) の NetCDF + 図 を生成
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "MCD ASCII テキストから surface forcing (theta_surf) と "
